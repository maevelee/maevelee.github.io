...
함수의 경우 우리가 직접 mysql dbms에 만들어서 사용할 수도 있고, mysql에서 기본적으로 제공하는 함수를 사용할 수도 있다. 오늘은 우선 기본 제공 함수의 사용법을 알아본다. 
...
```
라고 코드를 입력하면, a앞에 한 글자만 올 수 있고, a뒤로는 글자 길이를 상관하지 않는다. map, mapp, napppppp 모두 가능하다. a앞에 글자 하나만 오면 된다. 라는 의미이다.
...
저번 실습 때에도 잠깐 언급했지만, SQL에서 NULL값을 검색하려면 논리 연산자인 '='대신 'IS NULL'을 사용하여야 한다. 그렇지 않으면 정상적으로 값이 검색되지 않는다. 

IS NULL의 부정만은 NOT IS NULL이 아닌 IS NOT NULL이므로, 이 역시 주의하자!
